...
function iterate!{T<:AbstractFloat}(ls::MoreToraldoLineSearch{T},
    ...
    elseif stp > ls.stpmin
        q = -ls.ginit*stp
        r = f - (ls.finit - q)
        if (ls.strict ? r > zero(T) && q > zero(T) : r != zero(T))
...
function iterate!{T<:AbstractFloat}(ls::MoreThuenteLineSearch{T},
    ...
    if ls.brackt
        ...
        ls.lower = stp + XTRAPL(T)*(stp - ls.stx)
        ls.upper = stp + XTRAPU(T)*(stp - ls.stx)
    end
