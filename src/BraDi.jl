...
Module `OptimPack.BraDi` provides [``OptimPack.BraDi.minimize`](@ref) and
[`OptimPack.BraDi.maximize`](@ref) to find the global minimum or maximum of an univariate
function with the *"Bracket"* then *"Dig"* algorithm described in:

* Ferréol Soulez, Éric Thiébaut, Michel Tallon, Isabelle Tallon-Bosc and Paulo Garcia,
  *"Optimal a posteriori fringe tracking in optical interferometry"*, Proc. SPIE 9146,
  Optical and Infrared Interferometry IV, **91462Y** (July 24, 2014); doi:10.1117/12.2056590

"""
module BraDi

using TypeUtils: @public
@public ismonotonic, maximize, minimize
...
Specify keyword `periodic = true` if `f(x)` is a periodic function of period `b - a` with `a
= minimum(x)` and `b = maximum(x)`. If `periodic` is true then `a ≤ xm ≤ b` and `lo ≤ xm ≤
hi` hold but it is possible that `lo < a` and that `hi > b`.

Keywords `atol` and `rtol` may be used to specify the absolute and relative tolerances for
the precision of the solution (see [`OptimPack.Brent.fmin`](@ref)).

For example:

    BraDi.maximize(f, range(a, b; length=n))

performs a global search in the closed interval `[a,b]` which is sampled by `n` equally
spaced points.

This implements BraDi (*"Bracket"* then *"Dig"*) algorithm described in:

* Ferréol Soulez, Éric Thiébaut, Michel Tallon, Isabelle Tallon-Bosc and Paulo Garcia,
  *"Optimal a posteriori fringe tracking in optical interferometry"*, Proc. SPIE 9146,
  Optical and Infrared Interferometry IV, **91462Y** (July 24, 2014); doi:10.1117/12.2056590

See also [`OptimPack.BraDi.minimize`](@ref), [`OptimPack.Brent.fmax`](@ref), and
...
See also [`OptimPack.BraDi.maximize`](@ref), [`OptimPack.Brent.fmin`](@ref), and
[`OptimPack.Step.minimize`](@ref).
...
function minimize(::Type{T}, f, x::Points{<:Number}; periodic::Bool = false,
    ...
    # given.
    ismonotonic(x) || throw_bad_argument(
        "values of `x` are not strictly decreasing or increasing")
...
function __minimize(::Type{T}, f, ::Type{Tx}, x::Points{<:Number};
    ...
    if periodic
        ...
        xb, xc = x₁, x₂
        fb, fc = f₁, f₂
        k = stop - 1 # f(x[n]) = f(x[1]), so no needs to call f for the last point
    else
        xb = xc = x₁
        fb = fc = f₁
        k = stop
